    ...
    [
        new(
            ...
            [
                ...
                new("vpn-config", "Paste VPN configuration file", "secret-textarea", true, Secret: true, Help: "Paste the complete .ovpn file or WireGuard INI downloaded from your VPN provider. It is stored as a secret and is never shown again. When editing an installed gateway, leave this empty to keep its current configuration."),
                new("port-forwarding", "Incoming P2P port", "select", true, Help: "Required for good qBittorrent peer reachability. Proton requires a paid P2P server profile with NAT-PMP enabled.", Options: ["Required for qBittorrent/P2P", "Off (streaming only)"]),
