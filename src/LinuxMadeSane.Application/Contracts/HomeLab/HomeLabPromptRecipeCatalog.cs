...
public static class HomeLabPromptRecipeCatalog
{
    private const string OperatingContract =
        ...
        LMS must remain the source of truth for container state, storage, local access routes, health checks, and network routing. Reuse healthy installed infrastructure where the selected recipe requires it. If the user reports a web or playback symptom, an HTTP health check is not proof that the app works: inspect the actual local evidence, research current primary documentation only when that evidence is insufficient, then use the minimum repair and retest the reported path or capability. If required infrastructure is missing, direct me to the appropriate LMS form so credentials remain in protected fields. Inspect the result after changes and do not claim success unless the requested app is present, the reported path is verified, and the tool output says what changed or why no fix was possible.
        """;
    ...
    public static string BuildTroubleshootingPrompt(HomeLabAppInstallation installation)
    {
        ...
        return $"""
            ...
            Keep LMS as the source of truth. Do not use raw docker, docker compose, or shell commands to replace LMS-managed configuration. Never request or expose passwords, API keys, VPN profiles, temporary application passwords, or container log secrets. Start by inspecting the actual installation and its application configuration when relevant. If current documentation or an unfamiliar error is needed, use search_web and fetch_web_page selectively, preferring official vendor documentation and GitHub primary sources, then compare those findings with the inspected installation. For a confirmed non-secret setting fault, call repair_home_lab_app_config with one exact replacement from the inspected file; LMS will back it up, restart only that app, verify Docker health and local access, and roll back automatically if the check fails. Treat duplicate shared-namespace ports, stale VPN callbacks, incorrect VPN routing, and mismatched forwarded ports as LMS container or network faults. An explicit repair_home_lab_installation request may recreate an apparently healthy container when its health check does not cover the reported symptom; explain the evidence and request approval before changing it. A VPN-routed app is repaired alone unless the gateway security check proves the shared namespace is faulty. If I say manual image, environment, or volume edits caused the fault, set restoreContainerSettings to true. Inspect again afterward, retest the reported path or capability, and report the exact before/after change or the verified reason no fix was possible. Do not install another application or apply a HomeLab Recipe while troubleshooting this container.
            """;
    }
