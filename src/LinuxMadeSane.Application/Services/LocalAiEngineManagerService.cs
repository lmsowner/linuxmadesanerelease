...
public sealed class LocalAiEngineManagerService(
    ...
    ILmsConnectClientFeature connectClientFeature,
    IHttpClientFactory httpClientFactory) : ILocalAiEngineManagerService, ILocalAiEngineService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                throw new InvalidOperationException(
                    $"The shared LMS did not accept the connection ({(int)response.StatusCode}). Check the hostname and access key.");
            }

            using var document = JsonDocument.Parse(responseBody);
            var modelId = document.RootElement.TryGetProperty("data", out var data) &&
            ...
            {
                throw new InvalidOperationException("The shared LMS did not report an available Local AI model.");
            }
        ...
        {
            throw new InvalidOperationException("The shared LMS did not respond within 20 seconds. Check the hostname and network access.");
        }
        catch (JsonException)
        {
            throw new InvalidOperationException("The shared LMS returned an invalid Local AI model list.");
        }
        catch (HttpRequestException)
        {
            throw new InvalidOperationException("The shared LMS could not be reached. Check the hostname and HTTPS access.");
        }
    }

    private static Uri? ParsePeerLmsEndpoint(string? value)
    {
