    ...
    IAiChatRunQueue runQueue) : IAiChatOrchestrationService
{
        ...
        AiToolDefinition definition)
    {
        ...
        var settingsAction = request.RestoreContainerSettings
            ? " Restore the saved pre-edit image, environment, and volume settings first."
            : string.Empty;
        return new AiProposedActionProposal
        {
        ...
        AiToolDefinition definition)
    {
        ...
        return new AiProposedActionProposal
        {
            ...
            Description = $"Install, route, repair, and verify public HomeLab Recipe {request.PromptRecipeId} through LMS, listening on {request.ListenAddress} with {request.OutboundRoute} outbound traffic{gateway}.",
            ToolName = toolCall.ToolName,
            ProviderToolCallId = toolCall.ProviderToolCallId,
            ToolArgumentsJson = toolCall.ArgumentsJson,
            RiskLevel = definition.Approval.RiskLevel
