...
public sealed class XAiGrokAiProvider(
    string providerKey,
    ...
    IHttpClientFactory httpClientFactory) : IAiProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException($"The xAI Grok provider {Settings.DisplayName} does not have a usable API key.");
        }
    ...
    private JsonArray BuildMessages(AiProviderTurnRequest request)
    {
        ...
        foreach (var inputItem in request.InputItems)
        {
            switch (inputItem)
            {
                ...
                default:
                    throw new InvalidOperationException($"The provider input type {inputItem.GetType().Name} is not supported by xAI Grok.");
            }
        }
    ...
    private string ResolveModelId(AiProviderTurnRequest request)
    {
        ...
        {
            throw new InvalidOperationException($"The xAI Grok provider {Settings.DisplayName} does not have a model selected.");
        }
    ...
    {
        if (request.Thread.ProviderType != AiProviderType.XAi)
        {
            throw new InvalidOperationException("The xAI Grok provider adapter can only execute xAI Grok chat threads.");
        }
    ...
    internal Uri ResolveEndpoint()
    {
        ...
        {
            return new Uri("https://api.x.ai/v1/chat/completions", UriKind.Absolute);
        }
    ...
    internal static Uri ResolveOpenAiCompatibleEndpoint(string baseUrl, string pathSuffix)
    {
        ...
        foreach (var knownTerminalPath in new[] { "chat/completions", "models", "language-models", "responses" })
        {
        ...
        if (IsXAiApiHost(configured.Host) &&
            string.IsNullOrWhiteSpace(normalizedPath.Trim('/')))
        {
            return new Uri($"{normalizedBase}/v1/{pathSuffix}", UriKind.Absolute);
        }

        return new Uri($"{normalizedBase}/{pathSuffix}", UriKind.Absolute);
    }

    private static bool IsXAiApiHost(string host) =>
        host.Equals("api.x.ai", StringComparison.OrdinalIgnoreCase) ||
        host.EndsWith(".api.x.ai", StringComparison.OrdinalIgnoreCase);
    ...
    private static void EnsureResponseSucceeded(HttpResponseMessage response, string responseBody)
    {
        ...
        throw new InvalidOperationException(
            string.IsNullOrWhiteSpace(errorMessage)
                ? $"xAI Grok returned {(int)response.StatusCode} {response.ReasonPhrase}."
                : errorMessage);
    }
    ...
    internal static AiProviderTurnResult MapResult(string responseBody, AiProviderTurnRequest request)
    {
        var document = JsonNode.Parse(responseBody)?.AsObject()
            ?? throw new InvalidOperationException("xAI Grok returned an invalid JSON response.");

        var choices = document["choices"] as JsonArray ?? [];
        if (choices.Count == 0)
        {
            throw new InvalidOperationException("xAI Grok returned no choices.");
        }

        var choice = choices[0]?.AsObject()
                     ?? throw new InvalidOperationException("xAI Grok returned an invalid choice payload.");
        var message = choice["message"]?.AsObject()
                      ?? throw new InvalidOperationException("xAI Grok returned an invalid message payload.");
        var responseId = document["id"]?.GetValue<string>() ?? $"xai-{Guid.NewGuid():N}";
